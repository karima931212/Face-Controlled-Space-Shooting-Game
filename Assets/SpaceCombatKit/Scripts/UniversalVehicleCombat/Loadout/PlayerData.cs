	...
	public static class PlayerData 
	{
		...
		public static void SaveModuleLoadout(Vehicle modifiedVehicle, int vehicleIndex, PlayerItemManager itemManager)
		{
			...
			for (int i = 0; i < modifiedVehicle.ModuleMounts.Count; ++i)
			{
				...
				int index = itemManager.AllModulePrefabs.IndexOf(mountedModule.modulePrefab);
		...
		public static List<int> GetModuleLoadout(int vehicleIndex, PlayerItemManager itemManager)
		{
			...
			if (stringVal == "")
			{
				for (int i = 0; i < vehicle.ModuleMounts.Count; ++i)
				{
					if (vehicle.ModuleMounts[i].DefaultModules.Count > 0)
					{
						moduleIndexesByMount.Add(itemManager.AllModulePrefabs.IndexOf(vehicle.ModuleMounts[i].DefaultModules[0]));
					}
					else
					{
						moduleIndexesByMount.Add(-1);
			...
			for (int i = 0; i < moduleIndexesByMount.Count; ++i)
			{
				...
				if (moduleIndexesByMount[i] >= itemManager.AllModulePrefabs.Count)
				{
					moduleIndexesByMount[i] = Mathf.Clamp(moduleIndexesByMount[i], -1, itemManager.AllModulePrefabs.Count - 1);
				...
				if (itemManager.AllModulePrefabs[moduleIndexesByMount[i]] == null)
				{
				...
				IModule module = itemManager.AllModulePrefabs[moduleIndexesByMount[i]].GetComponent<IModule>();
				if (module == null)
				{ 
					moduleIndexesByMount[i] = -1;
				} 
